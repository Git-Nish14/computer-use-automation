...
```bash
# Unit tests — no browser, no API key needed (66 tests)
...
`capabilities/` and `evidence/` contain example files committed to this repo showing the expected output format:

| Path | Contents |
|---|---|
| `capabilities/lookup_savings_balance.json` | Example artifact (correct step refs, parameterized checkpoints) |
| `evidence/discovery_example/run.jsonl` | Example structured log from a discovery run |
| `evidence/replay_success_example/run.jsonl` | Example SUCCESS replay log |
| `evidence/replay_notfound_example/run.jsonl` | Example BUSINESS_OUTCOME log (member 99999) |

To generate genuine evidence from the current code, run Steps 1–3 above with your API key.
